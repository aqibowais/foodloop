...
class _EditListingScreenState extends ConsumerState<EditListingScreen> {
    ...
    try {
      ...
      if (listing != null && mounted) {
        setState(() {
          ...
          _servingsController.text = listing.servings.toString();
          _addressController.text = listing.address ?? '';
          _selectedFoodType = listing.foodType;
          _selectedExpiryDate = listing.expiryDate;
          _selectedExpiryTime = TimeOfDay.fromDateTime(listing.expiryDate);
          _existingImageUrls = List.from(listing.imageUrls);
          _isLoading = false;
        });
      } else if (mounted) {
        AppToast.error(context, 'Listing not found');
        Navigator.pop(context);
      }
    } catch (e) {
      if (mounted) {
        AppToast.error(context, 'Error loading listing: $e');
        Navigator.pop(context);
      }
    }
  }
  ...
  Future<void> _pickImages() async {
    try {
      ...
      if (images.isNotEmpty) {
        setState(() {
          _newImageFiles.addAll(images.map((xFile) => File(xFile.path)).toList());
        });
      }
    } catch (e) {
  ...
  Future<void> _selectExpiryDate() async {
    ...
    final pickedDate = await showDatePicker(
      context: context,
      initialDate: _selectedExpiryDate ?? now.add(const Duration(hours: 6)),
      firstDate: firstDate,
      lastDate: lastDate,
    ...
    if (pickedDate != null) {
      final pickedTime = await showTimePicker(
        context: context,
        initialTime: _selectedExpiryTime ?? TimeOfDay.now(),
        builder: (context, child) {
          return Theme(
            data: Theme.of(context).copyWith(
              colorScheme: const ColorScheme.dark(
  ...
  Future<void> _submitListing() async {
    ...
    if (success && mounted) {
      AppToast.success(context, 'Listing updated successfully!');
      Navigator.pop(context);
    } else if (mounted) {
